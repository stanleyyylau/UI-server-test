...
var common = {
  ...
  activateHead: function(){
    var helper = {
      showHead: function(){
        if(!($('.m-head').hasClass('J_sticky_head'))){
          $('.m-head').addClass('J_fadeIn');
          $('.taoli-logo').addClass('J_sticky_logo');
          setTimeout(function(){
            $('.m-head').addClass('J_sticky_head');
          },0)
        }
      },
      removeHead: function(){
        if($('.m-head').hasClass('J_sticky_head')){
          $('.m-head').removeClass('J_sticky_head');
          $('.m-head').removeClass('J_fadeIn');
          $('.taoli-logo').removeClass('J_sticky_logo');
         }
      }
    }
    function checkSc(){
    ...
    $(window).on("resize scroll", checkSc);
  }
}
